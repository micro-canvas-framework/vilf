...
Benchmarking enables fair comparison, capability mapping, and evidence-based resource allocation across labs, hubs, and networks. It provides comparative context rather than prescribing operational steps.
...
flowchart LR
  ...
  B --- T2
```
This diagram summarizes the benchmarking focus areas by tier without implying sequence, scoring, or progression.
:::
...
Normalization establishes comparability constraints, not targets or thresholds.
...
This sequence describes the analytical order of benchmarking, not an execution workflow:
